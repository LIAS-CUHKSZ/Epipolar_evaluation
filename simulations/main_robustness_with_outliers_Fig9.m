...
constant_t=[1 1 1]';
true_t_normal=constant_t/norm(constant_t);
true_E=[cross(constant_t,true_R(:,1)) cross(constant_t,true_R(:,2)) cross(constant_t,true_R(:,3))];
...
m=500;  % the number of points

outlier_rate=[0 0.02 0.04 0.06 0.08];

%%
% initilization
MSE_R_5pt=zeros(1,length(outlier_rate));
MSE_t_5pt=zeros(1,length(outlier_rate));
MSE_R_eigen=zeros(1,length(outlier_rate));
MSE_t_eigen=zeros(1,length(outlier_rate));
MSE_R_SDP=zeros(1,length(outlier_rate));
MSE_t_SDP=zeros(1,length(outlier_rate));
MSE_R_GNE=zeros(1,length(outlier_rate));
MSE_t_GNE=zeros(1,length(outlier_rate));
MSE_R=zeros(1,length(outlier_rate));
MSE_t=zeros(1,length(outlier_rate));
MSE_t_GN=zeros(1,length(outlier_rate));
MSE_R_GN=zeros(1,length(outlier_rate));
MSE_t_LM=zeros(1,length(outlier_rate));
MSE_R_LM=zeros(1,length(outlier_rate));
CRB_R=zeros(1,length(outlier_rate));
CRB_t=zeros(1,length(outlier_rate));
...
while counter<m
    ...
    if (x_temp2(3)>f) && (abs(x_temp2(1))<ix/2*x_temp2(3)/f) && (abs(x_temp2(2))<iy/2*x_temp2(3)/f)
        ...
        if rand<outlier_rate(j)
            z(:,counter)=[rand*ix-ix/2 rand*iy-iy/2]';
%             z(:,counter)= epipolar_outlier(true_R,true_t,y(:,counter),ix,iy);
        end
    end
end
y_h=[y;ones(1,m)];
...
%%
% ransac-5pt
[~, inliers] = ransacfitessmatrix(z_h, y_h, 0.00002);    % 0.001
y_h=y_h(:,inliers);
z_h=z_h(:,inliers);
...
[E_GNE,R_GNE,t_GNE] = GN_on_normalized_E(z_h,y_h,E_GNE,K);
[E_GNE,R_GNE,t_GNE] = GN_on_normalized_E(z_h,y_h,E_GNE,K);
[E_est,R_est,t_est,R_GN,t_GN,var_est] = consistent_essential_est_TLS_GN(z_h,y_h,K);
[E_LM,R_LM,t_LM]=robust_LM(z_h,y_h,R_5pt,t_5pt);
% [E_est,R_est,t_est,R_GN,t_GN] = robust_essential_est_iterative_TLS(z_h,y_h,K);


%%
% calculate MSE and statistics
MSE_R_5pt(j)=MSE_R_5pt(j)+norm(R_5pt-true_R,'fro')^2/monte_carlo;
MSE_t_5pt(j)=MSE_t_5pt(j)+norm(t_5pt-true_t_normal)^2/monte_carlo;
...
semilogy(outlier_rate,MSE_R_LM,'-o','color',[0.8627 0.1961 0.1373],'linewidth',2.5,'markersize',8);
hold on
semilogy(outlier_rate,MSE_R_eigen,'-s','color',[0.1882 0.2667 0.6275],'linewidth',2.5,'markersize',10);

semilogy(outlier_rate,MSE_R_SDP,'-^','color',[0.3725 0.7843 0.2941],'linewidth',2.5,'markersize',8);
semilogy(outlier_rate,MSE_R_GNE,'-x','color',[1.0000 0.6863 0],'linewidth',2.5,'markersize',10);
semilogy(outlier_rate,MSE_R_GN,'-d','color',[0.7255 0.2941 0.6863],'linewidth',2.5,'markersize',8);
% semilogy(outlier_rate,MSE_R_GN,'--d','color',[0.7255 0.2941 0.6863],'linewidth',2.5,'markersize',8);
legend('LM','Eigen','SDP','GN-E','CECME');
grid on
% set(gca, 'XTick',outlier_rate, 'XLim',[0 0.2]);
xlabel('Outlier rate')
ylabel('Rotation MSE')

figure
semilogy(outlier_rate,MSE_t_LM,'-o','color',[0.8627 0.1961 0.1373],'linewidth',2.5,'markersize',8);
hold on
semilogy(outlier_rate,MSE_t_eigen,'-s','color',[0.1882 0.2667 0.6275],'linewidth',2.5,'markersize',10);

semilogy(outlier_rate,MSE_t_SDP,'-^','color',[0.3725 0.7843 0.2941],'linewidth',2.5,'markersize',8);
semilogy(outlier_rate,MSE_t_GNE,'-x','color',[1.0000 0.6863 0],'linewidth',2.5,'markersize',10);
semilogy(outlier_rate,MSE_t_GN,'-d','color',[0.7255 0.2941 0.6863],'linewidth',2.5,'markersize',8);
% semilogy(outlier_rate,MSE_t_GN,'--d','color',[0.7255 0.2941 0.6863],'linewidth',2.5,'markersize',8);
legend('LM','Eigen','SDP','GN-E','CECME');
grid on
% set(gca, 'XTick',outlier_rate, 'XLim',[0 0.2]);
xlabel('Outlier rate')
ylabel('Translation MSE')
