...
bias_R_5pt_o=zeros(3,3);
bias_t_5pt_o=zeros(3,1);
...
while counter<m(j)
    ...
    if (x_temp2(3)>f) && (abs(x_temp2(1))<ix/2*x_temp2(3)/f) && (abs(x_temp2(2))<iy/2*x_temp2(3)/f)
        counter=counter+1;
        sigma_pix=0.5+rand;
        sigma(counter)=sigma_pix/f_pix;
        x(:,counter)=x_temp;
        y(:,counter)=y_temp;
        z_noisefree(:,counter)=x_temp2(1:2)/x_temp2(3)*f;
        z(:,counter)=x_temp2(1:2)/x_temp2(3)*f+randn(2,1)*sigma(counter);
    end
end
y_h=[y;ones(1,m(j))];
z_h=[z;ones(1,m(j))];
...
legend('LM','Eigen','SDP','GN-E','CECME init','CECME','CRB');
grid on
