...
MSE_t_GN1=zeros(length(sigma),length(m));
MSE_R_GN1=zeros(length(sigma),length(m));
MSE_t_GN2=zeros(length(sigma),length(m));
MSE_R_GN2=zeros(length(sigma),length(m));
MSE_t_GN3=zeros(length(sigma),length(m));
MSE_R_GN3=zeros(length(sigma),length(m));
MSE_t_GN5=zeros(length(sigma),length(m));
MSE_R_GN5=zeros(length(sigma),length(m));
...
%%
% pose estimation
[~,R_est,t_est,R_GN1,t_GN1,R_GN2,t_GN2,R_GN3,t_GN3,R_GN5,t_GN5,var_est] = consistent_essential_est_varied_GN_num(z_h,y_h,K);
...
MSE_R_GN1(l,j)=MSE_R_GN1(l,j)+norm(R_GN1-true_R,'fro')^2/monte_carlo;
MSE_t_GN1(l,j)=MSE_t_GN1(l,j)+norm(t_GN1-true_t_normal)^2/monte_carlo;
MSE_R_GN2(l,j)=MSE_R_GN2(l,j)+norm(R_GN2-true_R,'fro')^2/monte_carlo;
MSE_t_GN2(l,j)=MSE_t_GN2(l,j)+norm(t_GN2-true_t_normal)^2/monte_carlo;
MSE_R_GN3(l,j)=MSE_R_GN3(l,j)+norm(R_GN3-true_R,'fro')^2/monte_carlo;
MSE_t_GN3(l,j)=MSE_t_GN3(l,j)+norm(t_GN3-true_t_normal)^2/monte_carlo;
MSE_R_GN5(l,j)=MSE_R_GN5(l,j)+norm(R_GN5-true_R,'fro')^2/monte_carlo;
MSE_t_GN5(l,j)=MSE_t_GN5(l,j)+norm(t_GN5-true_t_normal)^2/monte_carlo;
...
hold on
loglog(m,MSE_R(1,:),'-','color',[0.8627 0.1961 0.1373],'linewidth',2.5,'markersize',8);

loglog(m,MSE_R_GN1(1,:),'--','color',[0.1882 0.2667 0.6275],'linewidth',2.5,'markersize',10);
loglog(m,MSE_R_GN2(1,:),':','color',[0.3725 0.7843 0.2941],'linewidth',2.5,'markersize',8);
loglog(m,MSE_R_GN3(1,:),'-.','color',[1.0000 0.6863 0],'linewidth',2.5,'markersize',10);
loglog(m,MSE_R_GN5(1,:),'--','color',[0.7255 0.2941 0.6863],'linewidth',2.5,'markersize',8);
legend('CRB','0 GN','1 GN','2 GN','3 GN','5 GN','fontsize',10);
grid on
...
hold on
loglog(m,MSE_t(1,:),'-','color',[0.8627 0.1961 0.1373],'linewidth',2.5,'markersize',8);
loglog(m,MSE_t_GN1(1,:),'--','color',[0.1882 0.2667 0.6275],'linewidth',2.5,'markersize',10);
loglog(m,MSE_t_GN2(1,:),':','color',[0.3725 0.7843 0.2941],'linewidth',2.5,'markersize',8);
loglog(m,MSE_t_GN3(1,:),'-.','color',[1.0000 0.6863 0],'linewidth',2.5,'markersize',10);
loglog(m,MSE_t_GN5(1,:),'--','color',[0.7255 0.2941 0.6863],'linewidth',2.5,'markersize',8);
legend('CRB','0 GN','1 GN','2 GN','3 GN','5 GN','fontsize',10);
grid on
set(gca, 'XTick',m, 'XLim',[10 3000]);
xlabel('Number of points','fontsize',12)
