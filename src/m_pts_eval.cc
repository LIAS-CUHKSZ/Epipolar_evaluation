...
void appendRes(const std::string &res_path, const std::string &res_name,
               double t, double r, int pt_num, double time,
               double est_vars = 0);
...
int main(int argc, char **argv) {
  if (argc != 6) {
    std::cout
        << "Usage: " << argv[0]
        << "  <datasetname>  <num_pts>  <pair_num1>  <pair_num2>  <sample_num>"
        << std::endl;
    return EXIT_FAILURE;
  }
  ...
  string dtset(argv[1]), pts_str(argv[2]), imgidx1(argv[3]), imgidx2(argv[4]),
      spn(argv[5]);
  if (imgidx1 == imgidx2) {
    std::cout << "Error: image indices must be different.Exit" << std::endl;
    return EXIT_FAILURE;
  }
  int pnum = stoi(pts_str), idx1 = stoi(imgidx1), idx2 = stoi(imgidx2),
      sample_num = stoi(spn);
  ...
  std::string res_path =
      "m-consistent-" + imgidx1 + "-" + imgidx2 + "-" + dtset;
  ...
  {
    if (images[idx2]->observations.count(
  ...
  if (covisible_num < pnum) {
    std::cout << "Warning: covisible points are less than the given target"
              << std::endl;
    std::cout << "All %d available pts would be used for eval" << covisible_num
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  /* ---------------------------remove outlier if
   * necessary--------------------------------------*/
  // @note the low res datasets have much more outlier(mis-match points pair)
  // than high-res one you can add definition REMOVE_OUTLIER when using low-res
  // one This option is recommended when you want to evaluate the m-consistency
  ...
  Matrix3d R1 =
      images[idx1]->global_T_image.linear().transpose().cast<double>();
  Matrix3d R_gt = ((images[idx1]->global_T_image.linear().transpose() *
                    images[idx2]->global_T_image.linear())
                       .cast<double>())
                      .transpose();
  Vector3d t_gt = -R_gt * R1 *
                  (images[idx2]->global_T_image.translation() -
                   images[idx1]->global_T_image.translation())
                      .cast<double>();
  if (t_gt.norm() < t_min || unskew(R_gt).norm() < r_min) // Apply thresholds
  {
    std::cout << "Error: translation too small or rotation too small. Exit"
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  double r_err_this_round[6] = {0}, t_err_this_round[6] = {0},
         time_cons_this_round[6] = {0};
  string method_name[6] = {"c_est", "E_GN",  "RANSAC-5pt",
                           "E_ELM", "E_SDP", "E_EIGEN"};
  double est_vars = 0;
  ...
  for (auto iter = 1; iter <= sample_num; iter++) {
    ...
    time_cons_this_round[0] += TIME_IT(est.GetPose(
        R_estimated, t_estimated, y_n, z_n, y_cv_pix, z_cv_pix, 1, -1, true););
    calcErr(t_err_this_round[0], r_err_this_round[0], R_gt, t_gt, R_estimated,
    ...
    Matrix3d R_r5pt;
    Vector3d t_r5pt;
    double init_time = TIME_IT(
        eigen2cv(cameras[images[idx1]->camera_id]->intrinsic, intrinsic_cv);
        E_cv = findEssentialMat(y_cv_pix, z_cv_pix, intrinsic_cv, cv::RANSAC,
                                0.999, 1.0);
        recoverPose(E_cv, y_cv_pix, z_cv_pix, intrinsic_cv, R_cv, t_cv);
        cv2eigen(R_cv, R_r5pt); cv2eigen(t_cv, t_r5pt););
    R_estimated = R_r5pt;
    t_estimated = t_r5pt;
    time_cons_this_round[2] += init_time;
    calcErr(t_err_this_round[2], r_err_this_round[2], R_gt, t_gt, R_estimated,
            t_estimated, use_lie); // Use calcErr
    ...
    time_cons_this_round[1] += TIME_IT(
        MGN.GetPose(R_r5pt, t_r5pt, y_cv_pix, z_cv_pix, y_n, z_n););
    calcErr(t_err_this_round[1], r_err_this_round[1], R_gt, t_gt, R_r5pt,
            t_r5pt, use_lie); // Use calcErr
    /* ↑------------------E Manifold GN------------------↑ */

    /* ↓------------------eigensolver estimator------------------↓ */
    // with the initial value given by RANSAC LMEDS
    EigenWrapper gv_esv(y_n, z_n);
    time_cons_this_round[5] +=
        TIME_IT(gv_esv.GetPose(R_estimated, t_estimated,
                               R_r5pt);) +
        init_time;
    calcErr(t_err_this_round[5], r_err_this_round[5], R_gt, t_gt, R_estimated,
            t_estimated, use_lie); // Use calcErr
    /* ↑------------------eigensolver estimator------------------↑ */

    lmSolverWrapper lm_solver(y_n, z_n);
    time_cons_this_round[3] +=
        TIME_IT(lm_solver.GetPose(R_estimated, t_estimated,
                                  R_r5pt, t_r5pt);) +
        init_time;
    calcErr(t_err_this_round[3], r_err_this_round[3], R_gt, t_gt, R_estimated,
            t_estimated, use_lie); // Use calcErr
    ...
    Matrix3d sdp_E;
    time_cons_this_round[4] += TIME_IT(
    ...
    calcErr(t_err_this_round[4], r_err_this_round[4], R_gt, t_gt, R_estimated,
            t_estimated, use_lie); // Use calcErr
    /* ↑------------------SDP on Essential Mat------------------↑ */
  }
  ...
  for (size_t i = 1; i < 6; ++i) {
    appendRes(res_path, method_name[i], t_err_this_round[i],
              r_err_this_round[i], pnum, time_cons_this_round[i]);
  }
  appendRes(res_path, method_name[0], t_err_this_round[0], r_err_this_round[0],
            pnum, time_cons_this_round[0], est_vars);
...
// 将结果写入csv文件
void appendRes(const std::string &res_path, const std::string &res_name,
               double t, double r, int pt_num, double time, double est_vars) {
